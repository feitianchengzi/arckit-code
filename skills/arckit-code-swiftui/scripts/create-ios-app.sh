...
while [ $i -le $# ]; do
    ...
    case "$arg" in
        ...
        --help|-h)
            ...
            echo "  --ios-version <version>   Minimum iOS version (default: 26.0)"
            echo "  --macos-version <version> Minimum macOS version (default: 26.0)"
            echo "  --watchos-version <version> Minimum watchOS version (default: 26.0)"
            echo "  --tvos-version <version>  Minimum tvOS version (default: 26.0)"
            echo "  --visionos-version <version> Minimum visionOS version (default: 26.0)"
            echo "  --output <path>           Output directory (default: current directory)"
            echo "  --help, -h                Show this help message"
            echo ""
            echo "Examples:"
            echo "  $0 --name MyApp --bundle-id com.example.MyApp"
            echo "  $0 --name MyApp --ios-version 26.0 --output ~/Projects"
            exit 0
            ;;
        *)
...
while true; do
    ...
    find "$PROJECT_DIR" -name "*{{*" 2>/dev/null | while read -r old_path; do
        ...
        new_path=$(echo "$old_path" | sed "s/{{PROJECT_NAME}}/$PROJECT_NAME/g")
...
PBX_TEST_FILE_REF_ID=$(generate_id "AF" "$PROJECT_NAME.tests.product")
PBX_EXCEPTION_SET_ID=$(generate_id "AF" "$PROJECT_NAME.exceptions")
PBX_ROOT_GROUP_ID=$(generate_id "AF" "$PROJECT_NAME.root")
PBX_TEST_ROOT_GROUP_ID=$(generate_id "AF" "$PROJECT_NAME.tests.root")
PBX_FRAMEWORKS_PHASE_ID=$(generate_id "AF" "$PROJECT_NAME.frameworks")
PBX_TEST_FRAMEWORKS_PHASE_ID=$(generate_id "AF" "$PROJECT_NAME.tests.frameworks")
...
PBX_TEST_TARGET_ID=$(generate_id "AF" "$PROJECT_NAME.tests.target")
PBX_TEST_CONTAINER_PROXY_ID=$(generate_id "AF" "$PROJECT_NAME.tests.proxy")
PBX_TEST_TARGET_DEPENDENCY_ID=$(generate_id "AF" "$PROJECT_NAME.tests.dependency")
PBX_TARGET_CONFIG_LIST_ID=$(generate_id "AF" "$PROJECT_NAME.target.config")
PBX_TEST_TARGET_CONFIG_LIST_ID=$(generate_id "AF" "$PROJECT_NAME.tests.target.config")
...
PBX_TEST_RESOURCES_PHASE_ID=$(generate_id "AF" "$PROJECT_NAME.tests.resources")
PBX_SOURCES_PHASE_ID=$(generate_id "AF" "$PROJECT_NAME.sources")
PBX_TEST_SOURCES_PHASE_ID=$(generate_id "AF" "$PROJECT_NAME.tests.sources")
...
PBX_TEST_TARGET_CONFIG_DEBUG_ID=$(generate_id "AF" "$PROJECT_NAME.tests.debug")
PBX_TEST_TARGET_CONFIG_RELEASE_ID=$(generate_id "AF" "$PROJECT_NAME.tests.release")
...
if [ -f "$PBX_PROJECT_FILE" ]; then
    if [[ "$OSTYPE" == "darwin"* ]]; then
        ...
        sed -i '' \
            ...
            -e "s/{{PBX_TEST_FILE_REF_ID}}/$PBX_TEST_FILE_REF_ID/g" \
            -e "s/{{PBX_EXCEPTION_SET_ID}}/$PBX_EXCEPTION_SET_ID/g" \
            -e "s/{{PBX_ROOT_GROUP_ID}}/$PBX_ROOT_GROUP_ID/g" \
            -e "s/{{PBX_TEST_ROOT_GROUP_ID}}/$PBX_TEST_ROOT_GROUP_ID/g" \
            -e "s/{{PBX_FRAMEWORKS_PHASE_ID}}/$PBX_FRAMEWORKS_PHASE_ID/g" \
            -e "s/{{PBX_TEST_FRAMEWORKS_PHASE_ID}}/$PBX_TEST_FRAMEWORKS_PHASE_ID/g" \
            ...
            -e "s/{{PBX_TEST_TARGET_ID}}/$PBX_TEST_TARGET_ID/g" \
            -e "s/{{PBX_TEST_CONTAINER_PROXY_ID}}/$PBX_TEST_CONTAINER_PROXY_ID/g" \
            -e "s/{{PBX_TEST_TARGET_DEPENDENCY_ID}}/$PBX_TEST_TARGET_DEPENDENCY_ID/g" \
            -e "s/{{PBX_TARGET_CONFIG_LIST_ID}}/$PBX_TARGET_CONFIG_LIST_ID/g" \
            -e "s/{{PBX_TEST_TARGET_CONFIG_LIST_ID}}/$PBX_TEST_TARGET_CONFIG_LIST_ID/g" \
            ...
            -e "s/{{PBX_TEST_RESOURCES_PHASE_ID}}/$PBX_TEST_RESOURCES_PHASE_ID/g" \
            -e "s/{{PBX_SOURCES_PHASE_ID}}/$PBX_SOURCES_PHASE_ID/g" \
            -e "s/{{PBX_TEST_SOURCES_PHASE_ID}}/$PBX_TEST_SOURCES_PHASE_ID/g" \
            ...
            -e "s/{{PBX_TEST_TARGET_CONFIG_DEBUG_ID}}/$PBX_TEST_TARGET_CONFIG_DEBUG_ID/g" \
            -e "s/{{PBX_TEST_TARGET_CONFIG_RELEASE_ID}}/$PBX_TEST_TARGET_CONFIG_RELEASE_ID/g" \
